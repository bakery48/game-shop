...
(function (root, factory) {
  ...
  const BALANCE = {
    ...
    skills: {
      ...
      scout:       { name: '目星',     from: null, pending: true,
                     ...
                     lotTier: 0.06 },
      sorting:     { name: '選り分け', from: null, pending: true,
                     desc: 'ロットの中身の状態が一段よくなりやすい',
                     lotCond: 0.20 },
      buzz:        { name: '拡散',     from: null, pending: true,
                     desc: '投稿1回が重くなる。評判の伸びも、持ち込みへの効きも上がる',
                     promo: 1.20 },
      charm:       { name: '愛想',     from: null, pending: true,
                     desc: '評判が上がるとき、上がり幅に上乗せされる（下がるときは変わらない）',
                     repGain: 0.35 },
      upsell:      { name: 'もう一声', from: null, pending: true,
                     desc: '指名客が2本目にも手を伸ばしやすくなる。手番は使わない',
                     secondBuy: 0.25 },
      shelving:    { name: '棚づくり', from: null, pending: true,
                     desc: '同じ広さの店に、あと5本ぶん仕舞える（並べる枠ではなく、抱えられる枠）',
                     shelfSlots: 5 },
    },
    ...
    promo: {
      ...
      // 1回あたりの評判。0.8では「撒くと最終残高が下がる」＝手番に見合わなかった。
      // 撒くほど得になる最小の値が2.5（測定は README）
      rep: 2.5,
  ...
  /**
   * 状態を一段よくする（交渉術「選り分け」）。効くのはロットだけ。
   * 単品入札やイベントは、そこに在る一本の状態が先に決まっている
   */
  function boostCond(st, source, mix) {
    if (source !== 'bulk' && source !== 'junk') return mix;
    const b = skill(st, 'lotCond');
    if (!b) return mix;
    const out = mix.slice();
    // 上から順に送る。先に下から動かすと、送った分をもう一度送ってしまう
  ...
  function rollCond(st, source, title) {
    ...
    const mix = boostCond(st, source, (c.mix && c.mix[source]) || c.mix.bulk);
    let r = st.rng();
  ...
  /** いま抱えられる本数。設備で増えた枠に、交渉術のぶんを足す */
  const shelfCap = st => st.cfg.shelfSlots + skill(st, 'shelfSlots');
  const freeSlots = st => shelfCap(st) - st.inv.length;
  /** いま並べられる本数。設備で増えた枠に、交渉術のぶんを足す */
  const displayCap = st => st.cfg.displaySlots + skill(st, 'displaySlots');
  const freeDisplay = st => displayCap(st) - st.inv.filter(i => i.display).length;
  ...
  function stats(st) {
    ...
    return {
      ...
      inventory: st.inv.length, slots: shelfCap(st), displaySlots: displayCap(st),
  ...
  function answer(st, yes) {
    ...
    if (c.type === 'buyer' && yes) {
      ...
      if (item) {
        ...
        if (st.rng() < st.cfg.buyerSecondItemChance + skill(st, 'secondBuy')) {
  ...
  /** 週の進みに応じてロットの中身を良くする */
  /**
   * ロットの区分を上に押し上げる（交渉術「目星」）。
   * 並品の一部を、上の区分へその比率のまま振り分ける。
   * 比率のままなので、処分品引取のようにレアが0の配分では0のまま増えない
   */
  function boostTier(st, mix) {
    const b = skill(st, 'lotTier');
    if (!b) return mix;
    const up = ['mid', 'rare', 'ultra'];
    const sum = up.reduce((a, k) => a + (mix[k] || 0), 0);
    if (!sum) return mix;
    const move = (mix.common || 0) * b;
    const out = Object.assign({}, mix);
    out.common = (mix.common || 0) - move;
    for (const k of up) out[k] = (mix[k] || 0) + move * (mix[k] || 0) / sum;
    return out;
  }

  function lotMix(st, spec) {
    if (!spec.earlyMix) return boostTier(st, spec.mix);
  ...
  function rep(st, delta) {
    ...
    // 上乗せは逓減の前に掛ける。逓減の後だと、評判が高いほど効かない二重の壁になる
    if (delta > 0) delta *= 1 + skill(st, 'repGain');
  ...
  function promoRate(st) {
    ...
    // 「拡散」は1回の投稿を重くする。頭打ちの位置は動かさず、そこへ早く届く
    return Math.min(1, (st.promo || 0) * (1 + skill(st, 'promo')) / c.cap);
  }
  ...
  function appraiseShelf(st) {
    ...
    d += Math.min(1, shelf.length / Math.max(1, displayCap(st))) * c.gain.shelfFull;
  ...
  return {
    ...
    setMemory, activeMemories, setSkill, skillOn, displayCap, shelfCap,
